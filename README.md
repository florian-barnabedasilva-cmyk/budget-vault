...
## Scanner IA Budget Vault

La fonctionnalité de scan IA analyse une image ou un PDF et préremplit uniquement les cinq champs existants : `type`, `amount`, `label`, `category`, `date`. La transaction n’est jamais enregistrée automatiquement : l’utilisateur valide d’abord le formulaire.

### Configuration

1. Copiez `.env.example` vers `.env`.
2. Renseignez `OPENAI_API_KEY`.
3. Lancez `npm run dev:full`.
4. Ouvrez l’application sur `http://localhost:3000`.

Le frontend appelle `/api/ai/analyze-receipt`; Vite le proxy vers le serveur Node sur le port 8787. En production, exécutez `npm run build` puis `npm start`; le serveur Node sert alors le dossier `dist` et l’API sur la même origine.

La clé OpenAI reste côté serveur et n’est jamais exposée au navigateur.
